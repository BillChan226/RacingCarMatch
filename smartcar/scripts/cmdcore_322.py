...
def initial_parameters():
    ...
    intrinsicMat = np.array([[572.3922, 0, 646.7288],
                            [0, 570.0095, 376.1008],
                            [0, 0, 1]])

    distortionCoe = np.array([0, 0, 0, 0, 0])
...
class command:
	...
	last_lane_base = -1
        ...
        while not rospy.is_shutdown():
            ...
            ##self.cap.set(CAP_PROP_FRAME_WIDTH,5000)
            if ret == True:
                if (not lidarLaunch):  
                    c1 = cv2.getTickCount()
                    kfObj = KalmanFilter()
