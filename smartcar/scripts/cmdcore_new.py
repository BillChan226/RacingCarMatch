...
def blind_detection(gray):
    ...
    #canny = cv2.dilate(canny, np.ones((10,10), np.uint8))
            ...
            if ret == True:
        	    ...
        	    #gray0 = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    ...
                    if ramp_control == True:
                        blind_detected, y_nearest = blind_detection(origin_thr)
                        if blind_detected == True:
                            print "__________ramp disappears_______________________"
                            cam_cmd.linear.x = 0
                        _, binary_gray = cv2.threshold(origin_thr, 0, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
                        binary_gray[:y_nearest, :] = 0
                        binary_warped = perspectiveTrans_slope(origin_thr)
                        margin = 30
                        histogram = np.sum(binary_warped[binary_warped.shape[0]*4/5:,:], axis=0) #//2
                    #print origin_thr.shape

                    else:
                        origin_thr[:origin_thr.shape[0]/3,:] = 0
