/*
使用说明：
1.本程序有两个函数暂时用不上，可以不管(已用“------------------”框出）：
    a.里程计：void updateOdometry(int steer_angular, int motor_angular, int round)
    b.手柄操作：void SimpleController::joyCallback(const sensor_msgs::Joy::ConstPtr& joy)

2.本程序可能要修改的参数有：
      a.cmdCallback函数中的scale_angle和ratio需要通过调参得到
	  b.cmdCallback中temp的上下限（可以通过ctrl+f检索“（*）”跳转），即舵机转角的上下限，需要通过手掰前轮转向测得
	  c.收发信息的topic名称，请ctrl+f检索"(A)""(B)""(C)""(D)"

3.收发信息的相关语句尤其是main()函数，基本遵循roscpp的收发信息的模板，具体可以参考 https://www.bilibili.com/video/av76568046?p=23 这个课程里topic_demo那一节

4.有关serial库的说明网上很难搜到，但相关程序的编写，尤其是SimpleController::SimpleController()这一构造函数的编写，参照了这一模板：https://www.cnblogs.com/TooyLee/p/6104863.html

5.还有一些小问题我没有看懂，已经注释在程序里了，但是应该不影响调参或者正常使用
*/
...
public:
  ...
  int odo_times = 0;
  void updateOdometry(int steer_angular, int motor_angular, int round);                                          //里程计
  //--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//构造函数
/*
从"ttyusb_port"中读取参数值
将消息发布到"odo_vel"这一topic上
订阅"joy" "cmd_vel"这两个topic上的消息，而接受到topic上的信息后的运行流程见joyCallback和cmdCallback两个回调函数
运行：
*/
  ...
  angular_(2)                                                                                            //??
{
	...
	*/
    ...
    ph_nh_.param<string>("ttyusb_port", ttyusb_port, string("/dev/ttyUSB0"));                          //为何多了个<string>?              //(A)
	/*
	...
	*/
    ...
    odo_pub_ = nh_.advertise<geometry_msgs::Twist>("odo_vel", 1);                                                                        //(B)
	/*
	发布话题，返回一个Publisher，负责广播topic
	尖括号“<>"中填写advertise的信息类型，该类型定义了消息的格式
    Publisher advertise (const std::string &topic, uint32_t queue_size, bool latch=false)
	topic:指定pubulisher往哪个topic发布消息        此处向"odo_vel"这个topic发布消息
	queue_size:发送的消息队列的长度    此处设定为1
	返回值为@publisher类型                                                                                  
	*/
    joy_sub_ = nh_.subscribe<sensor_msgs::Joy>("joy", 1, &SimpleController::joyCallback, this);                                        //(C)
    cmd_sub_ = nh_.subscribe<geometry_msgs::Twist>("cmd_vel", 1, &SimpleController::cmdCallback, this);                               //(D)
	/*
	...
	obj:回调函数的名称   回调函数的作用：当接收到发布者的信息后，需进行一系列的处理，处理过程就编写在回调函数中，                                                                            
	返回值为@subscriber类型                                                                                  
	*/
...
/*
geometry_msgs/Twist Message文件格式
Vector3  linear
Vector3  angular
其中，我们只会用到linear.x，linear.y以及angular.z
linear.x正方向--前
linear.y正方向--右
angular.z正方向--从上向下看绕竖直方向逆时针转动
*/
...
func:接收  包的控制信息
     若控制信息中x方向的速度为0，则执行stopRobot();
	   ...
	   其中，将小车的角速度转化为舵机的转角信号，将小车的前进速度转化为电机信号，存入data_buffer数组中，并将数组的信号发送出去
原理：
      小车角速度->舵机转角信号：temp = middle_angle - scale_angle*cmd->angular.z;
	                        temp:转角信号  middle_angle:舵机摆正的信号  scale_angle:调参而得的比例系数  cmd->angular.z:整车的角速度
      小车线速度->电机转速信号：speed = ratio * cmd->linear.x;  
	                          speed:电机转速信号 ratio:调参而得的比例系数 cmd->linear.x：整车的线速度
*/
void SimpleController::cmdCallback(const geometry_msgs::Twist::ConstPtr & cmd){//geometry_msgs/Twist Message文件格式请ctrl+f在本程序中搜索
    ...
    temp = middle_angle - scale_angle*cmd->angular.z; //- for right turn //+ for left turn             // 小车角速度->舵机转角信号  temp:转角信号  middle_angle:舵机摆正的信号  scale_angle:调参而得的比例系数  cmd->angular.z:整车的角速度

	//设置舵机左右转角的上下限，其中2793和1427这两个数据通过手掰前轮转向测得
...
//    if (fabs(cmd->linear.x - old_speed) >= 0.04)
  //  {
    ...
    //}
    //else
       speed = ratio * cmd->linear.x;//小车线速度->电机转速信号      speed:电机转速信号 ratio:调参而得的比例系数 cmd->linear.x：整车的线速度                                       
