...
def initial_parameters():
    ...
    intrinsicMat = np.array([[547.8874, 0, 645.326],             ##内参矩阵M1
                            [0, 549.7247, 396.244],
                            [0, 0, 1]])

    distortionCoe = np.array([0.3543, 0.2997, 0, 0, 0])        ##径向、切向畸变参数
    
    startx = 120         ##2D->2D   ，四点确定透视变换关系，startx\starty反应真实地面的左上点坐标，用于处理预瞄点
    starty = 50
    length_pers = 400
    width_pers = length_pers
    srcps = np.float32([[(204,296), (58,397), (545,367), (417,292)]])  ##(191,192), (32,278), (620,282), (450,190) #(196,213), (5,374), (634,370), (458,203) 
...

{
    ...
    kernel = np.ones((3,3),np.uint8)     ##3X3数组，数值为1，类型unit8
...
class command:
    def __init__(self):
        ...
        self.pubI = rospy.Publisher('images2', Image, queue_size=1)
        self.puborignialI = rospy.Publisher('orignial_images2', Image, queue_size=1)
...
##        part_A = 1
    ...
    if light_msg.red_or_green ==2 or light_msg.show_time == 1:
