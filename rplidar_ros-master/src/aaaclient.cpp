...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    sort(point_array1 , point_array1 + length/2 , cmp);
    sort(point_array2 , point_array2 + length/2 , cmp);
    finish=clock();
    total_time=(float)(finish-start)/CLOCKS_PER_SEC;
    ROS_INFO("TOTALTIME: %f",total_time);
    ROS_INFO("distance: %f",point_array2[0].rho);
    ROS_INFO("angle: %f",point_array2[0].theta);
