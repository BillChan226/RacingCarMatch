...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    for(int i = 0; i < count; i++) {
        ...
        if(i>0&&i<count/4)
        {
            if(y>0.1&&x>nearest_left)
            {
                nearest_left=x;
		
            }
        }
        if(i>count*3/4&&i<count)
        {
            if(y>0.1&&x<nearest_right)
            {
                nearest_right=x;
    ...
    {
	ROS_INFO("LEFT: %f",nearest_left);
	ROS_INFO("right: %f",nearest_right);    
    ...
    else if(judge_front(point_array[0].theta)&&in_rectangle(project_x,project_y,0.42,0.7))
    {
	ROS_INFO("FRONT");
