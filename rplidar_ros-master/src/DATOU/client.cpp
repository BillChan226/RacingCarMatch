...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
	...
	//float degree = RAD2DEG(rad);
	    ...
	    ROS_INFO("rho2: %f", point_array2[0].rho);
    }
    ...
    float project_x2 = point_array2[length].rho * sin(point_array2[0].theta);
    float project_y2 = point_array2[length].rho * cos(point_array2[0].theta);
    ...
    if(point_array1[0].rho<point_array2[0].rho)//the nearest obstacle is in front
    {
        ...
        {
            ROS_INFO("turn right");
            laser_cmd.angular.z = k * (safety + project_x1) + fabs(l * deta_y1);
            previous_turn[num_turn]=laser_cmd.angular.z;
            num_turn+=1;
        }
        else
	...
	ROS_INFO("rho1: %f", point_array1[0].rho);
        laser_cmd.angular.z=-previous_turn[num_turn];
        if(num_turn>0){num_turn-=1;}
    }
